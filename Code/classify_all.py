...
basepath = Path(basedir)   # makes the directory into a pathlib.WindowsPath object
...
label_names = sorted(item.name for item in basepath.glob('*/') if item.is_dir())
...
def preprocess_image(image_file):    # function to preprocess the image
    image_tensor = tf.image.decode_bmp(image_file, channels=1)  # ??not sure if 3 output channels are needed??
    image_tensor = tf.image.resize(image_tensor, img_shape) # ??i think this line is loosing a lot of information??
    image_tensor = image_tensor/255.0
    return image_tensor

def load_preprocess_image(image_dir):   # wrapper to load image the call preprocess_image()
    image_file = tf.io.read_file(image_dir)
    return preprocess_image(image_file)

'display an image'
import matplotlib.pyplot as plt
def displayimg(image_num): # possibly change this function slightly later to make the code more reusable
    image_dir = all_image_paths[image_num]
    image_label = all_image_labels[image_num]
    image_tensor = load_preprocess_image(image_dir)
    image_arr = tf.compat.v1.Session().run(image_tensor)  # !converts tf.Tensor object into np.array!
    image_arr = image_arr.reshape(img_shape)
    plt.imshow(image_arr)
    plt.xlabel('Image number ' + str(image_num) + ': ' + label_names[image_label].title())
    plt.show()
...
BATCH_SIZE = 32
ds = image_label_ds.shuffle(buffer_size=image_count)    # shuffles the entire dataset
...
model = tf.keras.Sequential([
    ...
    layers.Dense(len(label_names), activation='softmax') # classification layer
])
